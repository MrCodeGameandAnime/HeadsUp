...
public sealed class MainHudViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public string RepositoryName => _repository?.Name ?? "Select a repository";
    ...
    public string CiStatus => _ci is null ? "waiting for run" : CiText(_ci.Status);
    ...
    public string DetailCi => $"{CiStatus} {Runtime}".Trim();
    ...
    {
        if (!string.IsNullOrWhiteSpace(_preferences.RepositoryPath)) await RefreshAsync(true);
    ...
    public async Task RefreshAsync(bool forceCi = true)
    {
        ...
        try
        {
            ...
            var previousSha = _repository?.LocalHeadSha;
            _repository = await _git.GetStateAsync(_preferences.RepositoryPath, _preferences.Branch, _lifetime.Token);
            var shaChanged = !string.Equals(previousSha, _repository.LocalHeadSha, StringComparison.OrdinalIgnoreCase);
            if (shaChanged) _ci = CiState.Waiting(_repository.LocalHeadSha);
            NotifyAll();
    ...
    {
        if (_repository?.LocalHeadSha is not { Length: > 0 } sha || _ci?.IsTerminal == true && _ci.HeadSha == sha) return;
        var slug = await _git.GetGitHubSlugAsync(_repository.Path, _lifetime.Token);
    ...
    public Task<IReadOnlyList<string>> GetBranchesAsync() => string.IsNullOrWhiteSpace(_preferences.RepositoryPath) ? Task.FromResult<IReadOnlyList<string>>([]) : _git.GetBranchesAsync(_preferences.RepositoryPath, _lifetime.Token);
    ...
    private void NotifyAll() { foreach (var name in new[] { nameof(RepositoryName), nameof(Branch), nameof(LocalSha), nameof(RemoteSha), nameof(CiSha), nameof(FullLocalSha), nameof(FullRemoteSha), nameof(FullCiSha), nameof(LocalStatus), nameof(CommitAge), nameof(CiStatus), nameof(Workflow), nameof(Runtime), nameof(DetailCi), nameof(Jobs), nameof(CanOpenRun), nameof(CanOpenFailedLogs), nameof(AccentBrush), nameof(SurfaceOpacity), nameof(SurfaceBrush) }) OnChanged(name); }
